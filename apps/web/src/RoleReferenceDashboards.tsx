...
export function RoleReferenceSidebar({ kind, viewer, onNavigate, onSignOut }: { kind: RoleReferenceKind; viewer: User; onNavigate: (page: string) => void; onSignOut?: () => void }) {
  ...
  const nav: NavItem[] = kind === 'marketing-manager'
    ? [[active, IconHome, 'Dashboard'], ['Leads', IconUsers, 'Lead inbox'], ['Assignments', IconTarget, 'Assignments'], ['Reports', IconReportAnalytics, 'Reports'], ['Benchmark Board', IconChartBar, 'Benchmark Board'], ['Leaderboard', IconTrophy, 'Leaderboard'], ['Data quality', IconCircleCheck, 'Data quality'], ['Xaviar', IconRobot, 'Xaviar']]
    : kind === 'sales-manager'
      ? [[active, IconBriefcase, 'Dashboard'], ['Opportunities', IconBriefcase, 'Lead inbox'], ['Follow-ups', IconChecklist, 'Follow-ups'], ['Handoffs', IconUsers, 'Assignments'], ['Reports', IconReportAnalytics, 'Reports'], ['Benchmark Board', IconChartBar, 'Benchmark Board'], ['Leaderboard', IconTrophy, 'Leaderboard'], ['Data quality', IconCircleCheck, 'Data quality'], ['Xaviar', IconRobot, 'Xaviar']]
      : kind === 'marketing-agent'
        ? [[active, IconHome, 'Dashboard'], ['Leads', IconUsers, 'Lead inbox'], ['Follow-ups', IconChecklist, 'Follow-ups'], ['Reports', IconReportAnalytics, 'Reports'], ['My standing', IconTrophy, 'Leaderboard'], ['Xaviar', IconRobot, 'Xaviar']]
        : [[active, IconHome, 'Dashboard'], ['Leads', IconUsers, 'Lead inbox'], ['Follow-ups', IconChecklist, 'Follow-ups'], ['Assignments', IconTarget, 'Assignments'], ['Reports', IconReportAnalytics, 'Reports'], ['My standing', IconTrophy, 'Leaderboard'], ['Xaviar', IconRobot, 'Xaviar']];
  const [title, department] = roleLabel(kind);
  return <div className="admin-reference-sidebar role-reference-sidebar"><div className="admin-reference-brand"><IconSparkles className="admin-reference-spark" size={28} stroke={2.1} /><span>Elevanta <b>AI</b></span></div><nav aria-label={`${title} navigation`}>{nav.map(([label, Icon, page]) => <button key={label} className={label === active ? 'admin-reference-nav active' : 'admin-reference-nav'} onClick={() => onNavigate(page)}><Icon size={20} stroke={1.8} /><span>{label}</span></button>)}</nav><div className="admin-reference-sidebar-bottom"><button className="admin-reference-profile" onClick={onSignOut} aria-label="Sign out"><span>{initials(viewer.name)}</span><div><b>{viewer.name}</b><small>{title}<br />{department}</small></div></button></div></div>;
}
