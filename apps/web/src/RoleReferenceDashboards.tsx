import {
  IconActivityHeartbeat, IconAddressBook, IconAlertTriangle, IconArrowUpRight, IconBriefcase,
  IconBuilding, IconBulb, IconCalendar, IconChartBar, IconChecklist, IconCircleCheck, IconClock,
  IconCurrencyDollar, IconFilter, IconHelpCircle, IconHome, IconPlus, IconReportAnalytics,
  IconRobot, IconSettings, IconSparkles, IconTarget, IconTrendingUp, IconTrophy, IconUserCircle,
  IconUsers,
...
export function RoleReferenceSidebar({ kind, viewer, onNavigate, onUnavailable, onSignOut }: { kind: RoleReferenceKind; viewer: User; onNavigate: (page: string) => void; onUnavailable: (label: string) => void; onSignOut?: () => void }) {
  const active = kind.includes('agent') ? 'Workspace' : kind === 'sales-manager' ? 'Sales Command Center' : 'Command Center';
  type NavItem = [string, typeof IconHome, string?];
  const base: NavItem[] = kind === 'sales-manager'
    ? [[active, IconBriefcase, 'Dashboard'], ['Opportunities', IconBriefcase, 'Lead inbox'], ['Accounts', IconBuilding], ['Contacts', IconAddressBook], ['Activities', IconChecklist], ['Handoffs', IconUsers, 'Assignments'], ['Proposals', IconReportAnalytics, 'Reports'], ['Reports', IconChartBar, 'Reports'], ['Dashboards', IconChartBar, 'Dashboard'], ['Coaching', IconRobot, 'Xaviar'], ['Team', IconUsers, 'Leaderboard'], ['Insights', IconBulb, 'Reports']]
    : [[active, IconHome, 'Dashboard'], ['Leads', IconUsers, 'Lead inbox'], ['Accounts', IconBuilding], ['Contacts', IconAddressBook], ['Opportunities', IconBriefcase, 'Lead inbox'], ['Activities', IconChecklist], ['Campaigns', IconCalendar], ['Reports', IconReportAnalytics, 'Reports'], ['Dashboards', IconChartBar, 'Dashboard'], ['AI Agents', IconRobot, 'Xaviar'], ['Insights', IconBulb, 'Reports']];
  const nav = kind === 'marketing-agent' ? base.filter(([label]) => label !== 'Opportunities') : kind === 'sales-agent' ? base.filter(([label]) => label !== 'AI Agents') : base;
  const [title, department] = roleLabel(kind);
  return <div className="admin-reference-sidebar role-reference-sidebar"><div className="admin-reference-brand"><IconSparkles className="admin-reference-spark" size={28} stroke={2.1} /><span>Elevanta <b>AI</b></span></div><nav aria-label={`${title} navigation`}>{nav.map(([label, Icon, page]) => <button key={String(label)} className={label === active ? 'admin-reference-nav active' : 'admin-reference-nav'} onClick={() => page ? onNavigate(String(page)) : onUnavailable(String(label))}><Icon size={20} stroke={1.8} /><span>{label}</span></button>)}</nav><div className="admin-reference-sidebar-bottom"><button className="admin-reference-nav" onClick={() => onUnavailable('Settings')}><IconSettings size={20} /><span>Settings</span></button><button className="admin-reference-nav" onClick={() => onUnavailable('Help')}><IconHelpCircle size={20} /><span>Help</span></button><button className="admin-reference-profile" onClick={onSignOut} aria-label="Sign out"><span>{initials(viewer.name)}</span><div><b>{viewer.name}</b><small>{title}<br />{department}</small></div></button></div></div>;
}

function RoleTopbar({ kind, viewer, period, source, theme, onPeriod, onSource, onTheme, onCreate }: { kind: RoleReferenceKind; viewer: User; period: DashboardPeriod; source: string; theme: 'light' | 'dark'; onPeriod: (value: DashboardPeriod) => void; onSource: (value: string) => void; onTheme: (value: 'light' | 'dark') => void; onCreate: () => void }) {
...
function TrendPanel({ leads, title, onNavigate }: { leads: Lead[]; title: string; onNavigate: (page: string) => void }) {
  ...
  return <article className="role-reference-panel role-trend-panel"><div className="admin-reference-panel-heading"><h3>{title}</h3><span>Recorded events</span></div><div className="role-trend-legend"><span><i className="purple" />Activity</span><span><i className="cyan" />Routing</span></div><div className="role-trend-chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={data}><Line type="monotone" dataKey="activity" stroke="#8a5cf6" strokeWidth={2.2} dot={{ r: 2 }} isAnimationActive={false} /><Line type="monotone" dataKey="routing" stroke="#16b6bd" strokeWidth={2.2} dot={{ r: 2 }} isAnimationActive={false} /></LineChart></ResponsiveContainer></div><button className="admin-reference-footer-link" onClick={() => onNavigate('Reports')}>View trend details</button></article>;
}
...
function DisciplinePanel({ owners, followUps, overdue, onNavigate }: { owners: Array<{ name: string; work: number }>; followUps: ReturnType<typeof openFollowUps>; overdue: number; onNavigate: (page: string) => void }) {
  ...
  const ring = (data: typeof workload, center: string) => <div className="discipline-ring"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data} dataKey="value" innerRadius={26} outerRadius={43} stroke="none" isAnimationActive={false}>{data.map((item) => <Cell key={item.name} fill={item.color} />)}</Pie></PieChart></ResponsiveContainer><b>{center}</b></div>;
