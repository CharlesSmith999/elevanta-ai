...
function DashboardBoards({ viewer, leads, scope }: { viewer: User; leads: Lead[]; scope: DashboardScope }) {
  ...
  const personalRows = viewer.role === 'marketer' ? marketingRows : salesRows; const own = personalRows.find((row) => row.userId === viewer.id);
  const ownScore = viewer.role === 'marketer' ? own?.actionableLeadYield : own?.connectionRate;
