...
import {
  Activity, FollowUp, Lead, OpportunityStatus, Qualification, Role, User, canReassign, canUpdateLead,
  currentAssignment, dashboardFor, duplicateMatches, incorrectReviewState, isLeadIdentity, makeId,
  ownerId, qualificationLabels, seedLeads, statusLabels, users, validStatusTransition,
} from './domain';
...
export function App() {
  ...
  function updateStatus(lead: Lead, next: OpportunityStatus) {
    ...
    mutateLead(lead.id, (current) => ({ ...current, status: next, activities: [...current.activities, leadActivity(viewer.id, 'status', `Status changed from ${statusLabels[current.status]} to ${statusLabels[next]}.`)] }));
  ...
  function reassign(lead: Lead, event: FormEvent<HTMLFormElement>) {
    ...
    mutateLead(lead.id, (current) => ({ ...current, status: current.status === 'new' ? 'assigned' : current.status, assignments: current.assignments.map((assignment) => assignment.id === old?.id ? { ...assignment, endedAt: new Date().toISOString() } : assignment).concat({ id: makeId('assignment'), ownerId: nextOwnerId, assignedBy: viewer.id, at: new Date().toISOString(), visibility, reason }), activities: [...current.activities, leadActivity(viewer.id, 'assignment', `Assigned to ${nameFor(nextOwnerId)} with ${visibility === 'full_context' ? 'full context' : 'a fresh working view'}: ${reason}`)] }));
  ...
  function decideReview(lead: Lead, decision: 'confirmed_incorrect' | 'rejected' | 'merge_duplicate') {
    ...
    mutateLead(lead.id, (current) => ({ ...current, status: decision === 'rejected' ? 'follow_up_required' : decision === 'merge_duplicate' ? 'duplicate' : 'incorrect', routingPaused: false, incorrectReview: { state: decision, reviewerId: viewer.id, reason: decision === 'rejected' ? 'Admin review rejected the incorrect classification.' : 'Admin decision recorded.', decidedAt: new Date().toISOString() }, activities: [...current.activities, leadActivity(viewer.id, 'system', `Admin review decision: ${decision.replaceAll('_', ' ')}.`)] }));
  ...
  function createLead(event: FormEvent<HTMLFormElement>) {
    ...
    const lead: Lead = { id: makeId('lead'), name, phone: phone || undefined, email: email || undefined, source: source || 'Not available', marketingOwnerId: viewer.id, sourceDate: new Date().toISOString().slice(0, 10), status: owner ? 'assigned' : 'new', qualification: 'not_available', priority: 0, assignments: owner ? [{ id: makeId('assignment'), ownerId: owner, assignedBy: viewer.id, at: new Date().toISOString(), visibility: 'full_context', reason: 'Initial assignment' }] : [], activities: [leadActivity(viewer.id, 'system', 'Lead created in the test workspace.')], followUps: [], incorrectReports: [] };
  ...
  return <main className="app-shell">
    ...
    <section className="content"><header className="topbar"><div><p className="eyebrow">ELEVANTA AI / CRM TEST WORKSPACE</p><h1>{page}</h1></div><label>Test as a team member<select value={viewerId} onChange={(event) => { setViewerId(event.target.value); setSelectedId(undefined); }}><optgroup label="Admin & managers">{users.filter((user) => user.role === 'admin' || user.role === 'manager').map((user) => <option key={user.id} value={user.id}>{user.name} — {roleLabels[user.role]}</option>)}</optgroup><optgroup label="Marketing">{users.filter((user) => user.role === 'marketer').map((user) => <option key={user.id} value={user.id}>{user.name} — Marketing</option>)}</optgroup><optgroup label="Sales">{users.filter((user) => user.role === 'sales_agent').map((user) => <option key={user.id} value={user.id}>{user.name} — Sales Agent</option>)}</optgroup></select></label></header>
      ...
      {page === 'Dashboard' && <><PipelineIntelligence viewer={viewer} leads={visible} dashboard={dashboard} onSelect={select} /><section className="workspace-grid dashboard-workspace"><LeadTable leads={visible.slice(0, 5)} onSelect={select} onCreate={() => setShowCreate(true)} /><article className="panel xaviar"><span className="spark">✦ XAVIAR</span><h2>Coach’s note</h2><p>{dashboard.overdue ? `Resolve ${dashboard.overdue} overdue follow-up${dashboard.overdue === 1 ? '' : 's'} before adding more work.` : 'Every lead is currently on time. Keep the next follow-up date accurate.'}</p><p>{viewer.role === 'marketer' ? 'Watch MQL/SQL and duplicate rate to improve source quality.' : `Conversion from MQL/SQL to won is ${dashboard.conversionRate}% in this test workspace.`}</p></article></section></>}
      {page === 'Lead inbox' && <section className="inbox-layout"><LeadTable leads={visible} onSelect={select} onCreate={() => setShowCreate(true)} />{selected && <LeadDetail lead={selected} viewer={viewer} onStatus={updateStatus} onQualification={updateQualification} onAddNote={addNote} onAddFollowUp={addFollowUp} onCompleteFollowUp={completeFollowUp} onReassign={reassign} onReportIncorrect={reportIncorrect} onDecision={decideReview} />}</section>}
...
function PipelineIntelligence({ viewer, leads, dashboard, onSelect }: { viewer: User; leads: Lead[]; dashboard: ReturnType<typeof dashboardFor>; onSelect: (leadId: string) => void }) {
  ...
  return <section className="intelligence" aria-label="Pipeline intelligence dashboard">
    <div className="intelligence-heading"><div><span className="eyebrow">ROLE PERFORMANCE</span><h2>{copy.title}</h2><p>{copy.subtitle}</p></div><div className="intelligence-period"><IconActivityHeartbeat size={17} /> Last 14 days</div></div>
...
function RoleCharts({ viewer, leads, dashboard }: { viewer: User; leads: Lead[]; dashboard: ReturnType<typeof dashboardFor> }) {
  ...
  const detailOneTitle = isMarketer ? 'Lead source mix' : isSales ? 'My pipeline stages' : isManager ? 'Team follow-up health' : 'Company pipeline stages';
  const detailTwoTitle = isMarketer ? 'Quality risk watch' : isSales ? 'My risk watch' : isManager ? 'Team risk watch' : 'Company risk watch';
  const detailOneData = isMarketer ? sourceMix(leads) : isSales ? pipelineData(leads) : isManager ? followUpHealth(leads) : pipelineData(leads);
  const detailTwoData = riskData(leads);
  return <section className="insight-section" aria-label="Role-specific performance graphs"><div className="insight-heading"><div><span className="eyebrow">ROLE PERFORMANCE</span><h2>What you are working toward</h2></div><p>{insight}</p></div><div className="insight-grid"><BarChart title={firstTitle} subtitle={isMarketer ? 'Marketing-qualified and sales-qualified leads compared with final outcomes.' : 'Qualification movement and final outcomes from your visible leads.'} data={firstData} /><BarChart title={secondTitle} subtitle={isMarketer ? 'Use this to see whether lead quality turns into outcomes.' : isSales ? 'Follow-up timing is a controllable daily habit.' : isManager ? 'Shows where team capacity is currently allocated.' : 'Shows which marketer currently owns the visible lead flow.'} data={secondData} /></div><div className="insight-grid secondary-charts"><BarChart title={detailOneTitle} subtitle={isMarketer ? 'Shows which sources are producing your visible lead flow.' : isSales ? 'Shows exactly where your active work sits in the sales process.' : isManager ? 'Shows whether the team is keeping its commitments on time.' : 'Shows the shape of the visible company pipeline.'} data={detailOneData} /><BarChart title={detailTwoTitle} subtitle={isMarketer ? 'Use this to identify quality issues before sending more leads to Sales.' : isSales ? 'Resolve risks before they become lost opportunities.' : 'Use this to prioritise intervention before the risk grows.'} data={detailTwoData} /></div></section>;
}
...
function LeadDetail({ lead, viewer, onStatus, onQualification, onAddNote, onAddFollowUp, onCompleteFollowUp, onReassign, onReportIncorrect, onDecision }: { lead: Lead; viewer: User; onStatus: (lead: Lead, status: OpportunityStatus) => void; onQualification: (lead: Lead, qualification: Qualification) => void; onAddNote: (lead: Lead, event: FormEvent<HTMLFormElement>) => void; onAddFollowUp: (lead: Lead, event: FormEvent<HTMLFormElement>) => void; onCompleteFollowUp: (lead: Lead, id: string) => void; onReassign: (lead: Lead, event: FormEvent<HTMLFormElement>) => void; onReportIncorrect: (lead: Lead, event: FormEvent<HTMLFormElement>) => void; onDecision: (lead: Lead, decision: 'confirmed_incorrect' | 'rejected' | 'merge_duplicate') => void; }) {
  ...
  return <aside className="detail-panel"><div className="detail-heading"><div><span className="spark">LEAD DETAIL</span><h2>{lead.name}</h2><p>{lead.phone ?? 'No phone'} · {lead.email ?? 'No email'}</p></div><span className={`status ${lead.status}`}>{statusLabels[lead.status]}</span></div>
    <div className="detail-grid"><label>Status<select value={lead.status} disabled={!edit} onChange={(event) => onStatus(lead, event.target.value as OpportunityStatus)}>{statusOptions.map((status) => <option key={status} value={status} disabled={!validStatusTransition(lead.status, status)}>{statusLabels[status]}</option>)}</select></label><label>Qualification<select value={lead.qualification} disabled={!edit} onChange={(event) => onQualification(lead, event.target.value as Qualification)}>{qualificationOptions.map((level) => <option key={level} value={level}>{qualificationLabels[level]}</option>)}</select></label></div>
