...
import { RoleReferenceDashboard, RoleReferenceKind, RoleReferenceSidebar } from './RoleReferenceDashboards';
...
function WorkspaceApp({ onSignOut, session }: { onSignOut?: () => void; session?: Session }) {
  ...
  const roleReferenceKind: RoleReferenceKind | undefined = page !== 'Dashboard' ? undefined
    : viewer.role === 'manager' ? (viewer.department === 'marketing' ? 'marketing-manager' : 'sales-manager')
  ...
  return <main className={`app-shell theme-${theme} ${referenceDashboard ? 'admin-reference-shell role-reference-shell' : ''}`}>
    <aside className="sidebar">{adminReference ? <AdminReferenceSidebar viewer={viewer} onNavigate={(next) => { if (next === 'Dashboard') setDashboardScope('company'); setPage(next); setSelectedId(undefined); }} onUnavailable={(label) => setNotice(`${label} will be available from its dedicated workspace soon.`)} onSignOut={onSignOut} /> : roleReferenceKind ? <RoleReferenceSidebar kind={roleReferenceKind} viewer={viewer} onNavigate={(next) => { setPage(next); setSelectedId(undefined); }} onUnavailable={(label) => setNotice(`${label} will be available from its dedicated workspace soon.`)} onSignOut={onSignOut} /> : <><div className="brand"><span className="brand-mark">E</span><span>Elevanta <b>AI</b></span></div><p className="brand-subtitle">Sales, made clearer.</p>
      ...
      <div className="sidebar-bottom"><span>Working as</span><strong>{viewer.name}</strong><small>{viewer.role === 'manager' ? viewer.department === 'marketing' ? 'Marketing Manager' : 'Sales Manager' : roleLabels[viewer.role]}</small><button className="reset" onClick={resetDemo}>Reset test data</button>{onSignOut && <button className="reset" onClick={onSignOut}>Sign out</button>}</div></>}
    </aside>
    <section className="content">{!referenceDashboard && <><header className="topbar"><div><p className="eyebrow">ELEVANTA AI / CRM WORKSPACE</p><h1>{page}</h1></div><div className="topbar-actions"><div className="theme-switch" role="group" aria-label="Appearance"><button type="button" className={theme === 'light' ? 'theme-option active' : 'theme-option'} onClick={() => setTheme('light')} aria-pressed={theme === 'light'}><IconSun size={16} /> Light</button><button type="button" className={theme === 'dark' ? 'theme-option active' : 'theme-option'} onClick={() => setTheme('dark')} aria-pressed={theme === 'dark'}><IconMoonStars size={16} /> Dark</button></div><label>Test as a team member<select value={viewerId} onChange={(event) => { setViewerId(event.target.value); setSelectedId(undefined); setDashboardTeamMember('all'); }}><optgroup label="Admin & managers">{workspaceUsers.filter((user) => user.role === 'admin' || user.role === 'manager').map((user) => <option key={user.id} value={user.id}>{user.name} — {user.role === 'manager' ? user.department === 'marketing' ? 'Marketing Manager' : 'Sales Manager' : roleLabels[user.role]}</option>)}</optgroup><optgroup label="Marketing">{workspaceUsers.filter((user) => user.role === 'marketer').map((user) => <option key={user.id} value={user.id}>{user.name} — Marketing</option>)}</optgroup><optgroup label="Sales">{workspaceUsers.filter((user) => user.role === 'sales_agent').map((user) => <option key={user.id} value={user.id}>{user.name} — Sales Agent</option>)}</optgroup></select></label></div></header>
