...
import { IconActivityHeartbeat, IconAddressBook, IconAlertTriangle, IconArrowUpRight, IconBriefcase, IconBuilding, IconBulb, IconCalendar, IconChartBar, IconChartPieFilled, IconChecklist, IconCircleCheck, IconClock, IconCurrencyDollar, IconFilter, IconFlag, IconHelpCircle, IconHome, IconMoonStars, IconReportAnalytics, IconRobot, IconRocket, IconSettings, IconSun, IconTarget, IconTargetArrow, IconTrophy, IconTrendingUp, IconUserCircle, IconUsers } from '@tabler/icons-react';
...
function WorkspaceApp({ onSignOut, session }: { onSignOut?: () => void; session?: Session }) {
  ...
  const adminReference = page === 'Dashboard' && viewer.role === 'admin' && dashboardScope === 'company';

  return <main className={`app-shell theme-${theme} ${adminReference ? 'admin-reference-shell' : ''}`}>
    <aside className="sidebar">{adminReference ? <AdminReferenceSidebar viewer={viewer} onNavigate={(next) => { if (next === 'Dashboard') setDashboardScope('company'); setPage(next); setSelectedId(undefined); }} onUnavailable={(label) => setNotice(`${label} will be available from its dedicated workspace soon.`)} onSignOut={onSignOut} /> : <><div className="brand"><span className="brand-mark">E</span><span>Elevanta <b>AI</b></span></div><p className="brand-subtitle">Sales, made clearer.</p>
    ...
    <section className="content">{!adminReference && <><header className="topbar"><div><p className="eyebrow">ELEVANTA AI / CRM WORKSPACE</p><h1>{page}</h1></div><div className="topbar-actions"><div className="theme-switch" role="group" aria-label="Appearance"><button type="button" className={theme === 'light' ? 'theme-option active' : 'theme-option'} onClick={() => setTheme('light')} aria-pressed={theme === 'light'}><IconSun size={16} /> Light</button><button type="button" className={theme === 'dark' ? 'theme-option active' : 'theme-option'} onClick={() => setTheme('dark')} aria-pressed={theme === 'dark'}><IconMoonStars size={16} /> Dark</button></div><label>Test as a team member<select value={viewerId} onChange={(event) => { setViewerId(event.target.value); setSelectedId(undefined); setDashboardTeamMember('all'); }}><optgroup label="Admin & managers">{users.filter((user) => user.role === 'admin' || user.role === 'manager').map((user) => <option key={user.id} value={user.id}>{user.name} — {roleLabels[user.role]}</option>)}</optgroup><optgroup label="Marketing">{users.filter((user) => user.role === 'marketer').map((user) => <option key={user.id} value={user.id}>{user.name} — Marketing</option>)}</optgroup><optgroup label="Sales">{users.filter((user) => user.role === 'sales_agent').map((user) => <option key={user.id} value={user.id}>{user.name} — Sales Agent</option>)}</optgroup></select></label></div></header>
...
function AdminReferenceSidebar({ viewer, onNavigate, onUnavailable, onSignOut }: { viewer: User; onNavigate: (page: string) => void; onUnavailable: (label: string) => void; onSignOut?: () => void }) {
  const primary = [
    ['Command Center', IconHome, 'Dashboard'], ['Leads', IconUsers, 'Lead inbox'], ['Accounts', IconBuilding], ['Contacts', IconAddressBook], ['Opportunities', IconBriefcase], ['Activities', IconChecklist], ['Campaigns', IconCalendar], ['Reports', IconReportAnalytics, 'Reports'], ['Dashboards', IconChartBar, 'Dashboard'], ['AI Agents', IconRobot, 'Xaviar'], ['Insights', IconBulb],
  ] as const;
  return <div className="admin-reference-sidebar"><div className="admin-reference-brand"><IconSparkleMark /><span>Elevanta <b>AI</b></span></div><nav aria-label="Command center navigation">{primary.map(([label, Icon, page]) => <button key={label} className={label === 'Command Center' ? 'admin-reference-nav active' : 'admin-reference-nav'} onClick={() => page ? onNavigate(page) : onUnavailable(label)}><Icon size={20} stroke={1.8} /><span>{label}</span></button>)}</nav><div className="admin-reference-sidebar-bottom"><button className="admin-reference-nav" onClick={() => onUnavailable('Settings')}><IconSettings size={20} /><span>Settings</span></button><button className="admin-reference-nav" onClick={() => onUnavailable('Help')}><IconHelpCircle size={20} /><span>Help</span></button><div className="admin-reference-profile"><span>{initialsFor(viewer.name)}</span><div><b>{viewer.name}</b><small>Company Admin</small></div><IconArrowUpRight size={15} /></div>{onSignOut && <button className="admin-reference-signout" onClick={onSignOut}>Sign out</button>}</div></div>;
}

function IconSparkleMark() { return <IconTargetArrow className="admin-reference-spark" size={28} stroke={2.1} />; }

function AdminCommandCenter({ viewer, leads, dashboard, period, source, theme, onPeriod, onSource, onTheme, onNavigate }: { viewer: User; leads: Lead[]; dashboard: ReturnType<typeof dashboardFor>; period: DashboardPeriod; source: string; theme: 'light' | 'dark'; onPeriod: (value: DashboardPeriod) => void; onSource: (value: string) => void; onTheme: (value: 'light' | 'dark') => void; onNavigate: (page: string) => void }) {
  const financial = financialMetrics(leads);
  const stages = [
  ...
  const overdue = leads.flatMap((lead) => lead.followUps).filter((item) => item.status === 'open' && relativeDue(item) === 'Overdue').length;
  const dueSoon = leads.flatMap((lead) => lead.followUps).filter((item) => item.status === 'open' && relativeDue(item) === 'Today').length;
  const unassigned = leads.filter((lead) => !lead.assignments.length).length;
  const sourceRows = sourceMix(leads);
  const sourceMax = Math.max(1, ...sourceRows.map((row) => row.value));
  const history = intelligenceHistory(leads);
  const activityCount = leads.flatMap((lead) => lead.activities).length;
  const salesRows = leaderboardForSales(leads).slice(0, 5);
  const recentWins = leads.filter((lead) => lead.status === 'won').slice(0, 3);
  const recognition = [
    { Icon: IconRocket, label: 'Highest Result', person: salesRows[0] ? nameFor(salesRows[0].userId) : 'Not enough data', detail: salesRows[0] ? `${salesRows[0].connectionRate}% connection` : 'Need valid records' },
    { Icon: IconTarget, label: 'Consistency', person: salesRows[1] ? nameFor(salesRows[1].userId) : 'Not enough data', detail: salesRows[1] ? `${salesRows[1].sampleSize} visible leads` : 'Need valid records' },
    { Icon: IconTrendingUp, label: 'Quality Champion', person: sourceRows[0]?.label ?? 'Not enough data', detail: sourceRows[0] ? `${sourceRows[0].value} visible leads` : 'Need valid records' },
  ];
  const metricCards = [
  ...
  const execution = [
    { label: 'Recorded activity', value: activityCount, tone: 'green' }, { label: 'Connected leads', value: leads.filter((lead) => lead.status === 'connected').length, tone: 'green' }, { label: 'Proposals sent', value: leads.filter((lead) => lead.status === 'proposal_sent').length, tone: 'green' }, { label: 'Conversion rate', value: `${dashboard.conversionRate}%`, tone: dashboard.conversionRate ? 'green' : 'muted' },
  ];
  const risks = [
    { label: 'High priority', note: 'Overdue follow-ups', value: overdue, Icon: IconFlag, tone: 'red' }, { label: 'Medium priority', note: 'Due today', value: dueSoon, Icon: IconClock, tone: 'amber' }, { label: 'Low priority', note: 'Unassigned leads', value: unassigned, Icon: IconAlertTriangle, tone: 'yellow' },
  ];
  return <div className="admin-reference-dashboard"><header className="admin-reference-topbar"><h1>Company Command Center</h1><div className="admin-reference-controls"><label className="admin-reference-select"><IconCalendar size={17} /><select aria-label="Period" value={period} onChange={(event) => onPeriod(event.target.value as DashboardPeriod)}>{Object.entries(dashboardPeriodLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="admin-reference-select"><IconFilter size={17} /><select aria-label="Source" value={source} onChange={(event) => onSource(event.target.value)}><option value="all">All Sources</option>{sourceOptions.map((value) => <option key={value} value={value}>{value}</option>)}</select></label><div className="admin-reference-theme" role="group" aria-label="Appearance"><span>Dark</span><button type="button" aria-label="Toggle dark mode" aria-pressed={theme === 'dark'} onClick={() => onTheme(theme === 'dark' ? 'light' : 'dark')}><i /></button><span>Light</span></div><div className="admin-reference-avatar" aria-label={`${viewer.name} profile`}>{initialsFor(viewer.name)}</div></div></header>
    ...
    <section className="admin-reference-section" aria-label="Performance"><div className="admin-reference-section-heading"><h2>Performance</h2><button onClick={() => onNavigate('Reports')}>Improve <IconArrowUpRight size={15} /></button></div><div className="admin-reference-performance-grid"><article className="admin-reference-panel admin-funnel-panel"><div className="admin-reference-panel-heading"><h3>Conversion funnel</h3><span>Selected period</span></div><div className="admin-reference-funnel-body"><div className="admin-funnel-chart"><ResponsiveContainer width="100%" height="100%"><FunnelChart><Tooltip contentStyle={{ background: 'var(--surface)', borderColor: 'var(--border)', color: 'var(--text)' }} /><Funnel dataKey="value" data={stages} isAnimationActive={false}><LabelList dataKey="name" position="right" fill="var(--text)" fontSize={10} /></Funnel></FunnelChart></ResponsiveContainer></div><div className="admin-reference-funnel-values">{stages.map((stage) => <div key={stage.name}><span>{stage.name}</span><b>{stage.value}</b><small>{leads.length ? `${Math.round((stage.value / leads.length) * 100)}%` : 'Not available'}</small></div>)}<footer><span>Conversion rate (Won)</span><b>{dashboard.conversionRate}%</b></footer></div></div></article>
      ...
      <article className="admin-reference-panel"><div className="admin-reference-panel-heading"><h3>Source quality</h3><span>By lead volume</span></div><div className="admin-reference-source-list">{sourceRows.length ? sourceRows.map((row) => <div key={row.label}><span>{row.label}</span><i><b style={{ width: `${Math.max(8, row.value / sourceMax * 100)}%` }} /></i><strong>{row.value}</strong></div>) : <p>Not enough data</p>}</div><button className="admin-reference-footer-link" onClick={() => onNavigate('Reports')}>View full source report</button></article>
      <article className="admin-reference-panel admin-execution-panel"><div className="admin-reference-panel-heading"><h3>Sales execution</h3><span>Recorded activity</span></div>{execution.map((item, index) => <div className="admin-reference-execution-row" key={item.label}><div><span>{item.label}</span><b>{item.value}</b></div><div className={`admin-reference-spark tone-${item.tone}`}><ResponsiveContainer width="100%" height="100%"><LineChart data={history.slice(Math.max(0, history.length - 7))}><Line type="monotone" dataKey="activity" stroke="var(--accent-strong)" strokeWidth={2.2} dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div></div>)}<button className="admin-reference-footer-link" onClick={() => onNavigate('Reports')}>View sales dashboard</button></article></div></section>
    <section className="admin-reference-section" aria-label="Momentum and recognition"><div className="admin-reference-section-heading"><h2>Momentum &amp; Recognition</h2></div><div className="admin-reference-momentum-grid"><article className="admin-reference-panel"><div className="admin-reference-panel-heading"><h3>Top performers</h3><span>Visible sample</span></div><div className="admin-reference-rank-list">{salesRows.length ? salesRows.map((row, index) => <div key={row.userId}><i>{initialsFor(nameFor(row.userId))}</i><b>{nameFor(row.userId)}</b><span>{row.sampleSize} leads</span><strong>{row.connectionRate}%</strong></div>) : <p>Not enough data</p>}</div><button className="admin-reference-footer-link" onClick={() => onNavigate('Leaderboard')}>View leaderboard</button></article>
