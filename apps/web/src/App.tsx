...
function CreateLead({ error, saving, viewer, salesAgents, crmReady, onReconnect, onClose, onSubmit }: { error: string; saving: boolean; viewer: User; salesAgents: User[]; crmReady: boolean; onReconnect?: () => void; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) { const allowed = viewer.role === 'admin' || viewer.role === 'marketer'; return <div className="modal-backdrop" role="presentation"><form className="modal lead-form-modal" onSubmit={onSubmit}><div className="panel-heading"><div><span className="spark dark">NEW LEAD</span><h2>Create a lead</h2></div><button type="button" className="quiet" onClick={onClose}>Close</button></div>{allowed ? <>{error && <p className="warning" role="alert">{error}</p>}<p>A record only becomes a lead when it has a name plus a phone number or email address.</p>{!crmReady && <div className="warning crm-reconnect-warning"><span>The CRM connection is unavailable. Reconnect before creating a lead so no assignment is lost.</span>{onReconnect && <button type="button" className="quiet" onClick={() => { onClose(); onReconnect(); }}>Reconnect</button>}</div>}<div className="detail-grid"><label>Name<input name="name" required /></label><label>Source<select name="source" defaultValue="" required><option value="" disabled>Choose source</option>{sourceOptions.map((source) => <option key={source} value={source}>{source}</option>)}</select></label><label>Lead category<select name="category" defaultValue="not_available" required>{leadCategoryOptions.map((category) => <option key={category} value={category}>{leadCategoryLabels[category]}</option>)}</select></label><label>Phone<input name="phone" type="tel" inputMode="tel" /></label><label>Email<input name="email" type="email" inputMode="email" /></label><label>Initial sales owner<select name="owner" defaultValue=""><option value="">Leave unassigned</option>{salesAgents.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label><label className="form-span">Description<textarea name="description" rows={5} maxLength={4000} placeholder="Add the lead request, project needs, background, or other useful context." /></label></div><button className="primary" disabled={!crmReady || saving}>{saving ? 'Saving…' : 'Create and save lead'}</button></> : <p className="warning">Only Admin and Marketing can create leads.</p>}</form></div>; }
