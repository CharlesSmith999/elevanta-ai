...
function WorkspaceApp({ onSignOut, session }: { onSignOut?: () => void; session?: Session }) {
  ...
  return <main className={`app-shell theme-${theme} admin-reference-shell role-reference-shell ${mobileNavOpen ? 'nav-open' : ''}`}>
    ...
    {showCreate && <CreateLead viewer={viewer} salesAgents={workspaceDirectory.filter((user) => user.role === 'sales_agent')} crmReady={!session || remoteLoaded} onReconnect={session ? onSignOut : undefined} onClose={() => setShowCreate(false)} onSubmit={createLead} />}
  </main>;
}
