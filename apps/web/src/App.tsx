...
function DashboardBoards({ viewer, leads, scope }: { viewer: User; leads: Lead[]; scope: DashboardScope }) {
  const salesRows = leaderboardForSales(leads); const marketingRows = leaderboardForMarketing(leads);
  const management = canViewNamedLeaderboard(viewer);
  const personalRows = viewer.role === 'marketer' ? marketingRows : salesRows;
  const own = personalRows.find((row) => row.userId === viewer.id);
  const benchmark = median((viewer.role === 'marketer' ? leaderboardForMarketing(seedLeads) : leaderboardForSales(seedLeads)).map((row) => viewer.role === 'marketer' ? row.actionableLeadYield : row.connectionRate));
  ...
  const topRows = (viewer.role === 'admin' && scope === 'company' ? [...salesRows, ...marketingRows] : scope === 'marketing' ? marketingRows : salesRows).slice(0, 4);
  return <section className="dashboard-boards" aria-label="Dashboard boards and operational watchlist">
    ...
    <div className="dashboard-board-grid">
      <article className="panel board-preview"><div className="board-preview-heading"><div><span className="eyebrow">{management ? 'LEADERBOARD' : 'PRIVATE STANDING'}</span><h3>{management ? (viewer.role === 'admin' && scope === 'company' ? 'Top visible performers' : scope === 'marketing' ? 'Marketing team standing' : 'Sales team standing') : 'Your result vs team benchmark'}</h3></div><span className="board-chip">{management ? 'Named view' : 'Private'}</span></div>{management ? <div className="mini-ranking">{topRows.length ? topRows.map((row) => <div key={row.userId} className="mini-ranking-row"><b>{nameFor(row.userId)}</b><span>{row.sampleSize} leads</span><strong>{percent(viewer.role === 'admin' && scope === 'company' ? ('connectionRate' in row ? row.connectionRate : row.actionableLeadYield) : scope === 'marketing' ? row.actionableLeadYield : row.connectionRate)}</strong></div>) : <p className="empty">Not enough data to rank this team.</p>}</div> : <div className="personal-standing"><div><small>My sample</small><b>{own?.sampleSize ?? 'Not enough data'}</b></div><div><small>My score</small><b>{percent(viewer.role === 'marketer' ? own?.actionableLeadYield : own?.connectionRate)}</b></div><div><small>Team benchmark</small><b>{percent(benchmark)}</b></div></div>}</article>
