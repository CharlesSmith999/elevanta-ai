...
function LeadTable({ leads, onSelect, onCreate, onViewAll }: { leads: Lead[]; onSelect: (id: string) => void; onCreate: () => void; onViewAll?: () => void }) { return <article className="panel leads-panel"><div className="panel-heading"><div><span className="eyebrow">LEAD INBOX <em className="preview-badge">22 new</em></span><h2>Ready for the next action</h2><p>Lead score is a visual preview for now; the scoring model will be built later.</p></div><div className="lead-table-actions"><button className="primary" onClick={onCreate}>Add lead</button>{onViewAll && <button className="quiet" onClick={onViewAll}>View all leads <IconArrowUpRight size={15} /></button>}</div></div><div className="lead-table"><div className="lead-row header"><span>Lead</span><span>Score</span><span>Owner</span><span>Status</span><span>Next action</span></div>{leads.length ? leads.map((lead) => { const next = lead.followUps.find((followUp) => followUp.status === 'open'); const score = leadScorePreview(lead); return <button className="lead-row lead-button" key={lead.id} onClick={() => onSelect(lead.id)}><strong><span className="lead-identity"><span className="avatar avatar-lead">{initialsFor(lead.name)}</span><span>{lead.name}<small>{lead.source}</small></span></span></strong><span className={`lead-score ${score >= 75 ? 'good' : score < 50 ? 'risk' : 'watch'}`}>{score}</span><span className="owner-identity"><span className="avatar">{initialsFor(nameFor(ownerId(lead)))}</span>{nameFor(ownerId(lead))}</span><span className={`status ${lead.status}`}>{statusLabels[lead.status]}</span><span>{relativeDue(next)}</span></button>; }) : <p className="empty">No leads are visible for this user.</p>}</div></article>; }
