...
import {
  ...
  ownerId, qualificationLabels, seedLeads, sourceOptions, statusLabels, users, validStatusTransition, validWonFinancials,
} from './domain';
...
export function App() {
  ...
  function markWon(lead: Lead, event: FormEvent<HTMLFormElement>) {
    ...
    mutateLead(lead.id, (current) => ({ ...current, status: 'won', totalProjectCost: total, upfrontPaymentAmount: upfront, wonAt: new Date().toISOString(), activities: [...current.activities, leadActivity(viewer.id, 'status', `Marked Won. Total project cost: ${total}; upfront payment: ${upfront}.`)] }));
  ...
  function createLead(event: FormEvent<HTMLFormElement>) {
    ...
    const lead: Lead = { id: makeId('lead'), name, phone: phone || undefined, email: email || undefined, source, marketingOwnerId: viewer.id, sourceDate: new Date().toISOString().slice(0, 10), status: owner ? 'assigned' : 'new', qualification: 'not_available', priority: 0, assignments: owner ? [{ id: makeId('assignment'), ownerId: owner, assignedBy: viewer.id, at: new Date().toISOString(), visibility: 'full_context', reason: 'Initial assignment' }] : [], activities: [leadActivity(viewer.id, 'system', 'Lead created in the test workspace.')], followUps: [], incorrectReports: [] };
