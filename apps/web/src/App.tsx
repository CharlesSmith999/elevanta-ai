...
import { IconActivityHeartbeat, IconAlertTriangle, IconArrowUpRight, IconBriefcase, IconBulb, IconCalendar, IconChartBar, IconChartPieFilled, IconChecklist, IconChevronDown, IconCircleCheck, IconClock, IconCurrencyDollar, IconFilter, IconFlag, IconHome, IconMoonStars, IconReportAnalytics, IconRobot, IconRocket, IconSparkles, IconSun, IconTarget, IconTargetArrow, IconTrophy, IconTrendingUp, IconUserCircle, IconUsers } from '@tabler/icons-react';
...
function WorkspaceApp({ onSignOut, session }: { onSignOut?: () => void; session?: Session }) {
  ...
  return <main className={`app-shell theme-${theme} ${referenceDashboard ? 'admin-reference-shell role-reference-shell' : ''}`}>
    <aside className="sidebar">{adminReference ? <AdminReferenceSidebar viewer={viewer} onNavigate={(next) => { if (next === 'Dashboard') setDashboardScope('company'); setPage(next); setSelectedId(undefined); }} onSignOut={onSignOut} /> : roleReferenceKind ? <RoleReferenceSidebar kind={roleReferenceKind} viewer={viewer} onNavigate={(next) => { setPage(next); setSelectedId(undefined); }} onSignOut={onSignOut} /> : <><div className="brand"><span className="brand-mark">E</span><span>Elevanta <b>AI</b></span></div><p className="brand-subtitle">Sales, made clearer.</p>
...
function AdminReferenceSidebar({ viewer, onNavigate, onSignOut }: { viewer: User; onNavigate: (page: string) => void; onSignOut?: () => void }) {
  const primary = [
    ['Command Center', IconHome, 'Dashboard'], ['Leads', IconUsers, 'Lead inbox'], ['Follow-ups', IconChecklist, 'Follow-ups'], ['Assignments', IconTarget, 'Assignments'], ['Reports', IconReportAnalytics, 'Reports'], ['Benchmark Board', IconChartBar, 'Benchmark Board'], ['Leaderboard', IconTrophy, 'Leaderboard'], ['Data quality', IconCircleCheck, 'Data quality'], ['Review queue', IconFlag, 'Review queue'], ['User management', IconUserCircle, 'User management'], ['Xaviar', IconRobot, 'Xaviar'],
  ] as const;
  return <div className="admin-reference-sidebar"><div className="admin-reference-brand"><IconSparkleMark /><span>Elevanta <b>AI</b></span></div><nav aria-label="Command center navigation">{primary.map(([label, Icon, page]) => <button key={label} className={label === 'Command Center' ? 'admin-reference-nav active' : 'admin-reference-nav'} onClick={() => onNavigate(page)}><Icon size={20} stroke={1.8} /><span>{label}</span></button>)}</nav><div className="admin-reference-sidebar-bottom"><button className="admin-reference-profile" onClick={onSignOut} aria-label="Sign out"><span>{initialsFor(viewer.name)}</span><div><b>{viewer.name}</b><small>Company Admin</small></div><IconChevronDown size={15} /></button></div></div>;
}
