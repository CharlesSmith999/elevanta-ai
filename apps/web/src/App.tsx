...
function WorkspaceApp({ onSignOut, session }: { onSignOut?: () => void; session?: Session }) {
  ...
  async function createLead(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (creatingLead) return; setCreateError(''); const formElement = event.currentTarget; const form = new FormData(formElement); const name = String(form.get('name') ?? '').trim(); const phone = String(form.get('phone') ?? '').trim(); const email = String(form.get('email') ?? '').trim(); const source = String(form.get('source') ?? '').trim(); const category = String(form.get('category') ?? 'not_available') as NonNullable<Lead['category']>; const description = String(form.get('description') ?? '').trim(); const owner = String(form.get('owner') ?? '');
    if (!isLeadIdentity(name, phone, email)) return setCreateError('A lead needs a name plus a phone number or email address.');
    if (viewer.role !== 'admin' && viewer.role !== 'marketer') return setCreateError('Only Admin and Marketing can create leads.');
    if (!sourceOptions.includes(source as typeof sourceOptions[number])) return setCreateError('Choose an approved source. Use Other when the source is unknown.');
    if (!leadCategoryOptions.includes(category)) return setCreateError('Choose an approved lead category.');
    ...
    if (session) {
      if (!remoteLoaded) { setCreateError('The lead was not created because the CRM connection is unavailable. Sign in again and retry.'); return; }
      ...
      try {
        ...
        formElement.reset();
        try {
          setLeads(await loadRemoteLeads(session));
          setNotice(duplicate ? 'Lead saved to the CRM and flagged as a duplicate candidate for review.' : owner ? 'Lead saved to the CRM and assigned successfully.' : 'Lead saved to the CRM without an assignment.');
        } catch {
          setRemoteLoaded(false);
          setConnectionError('The lead was saved, but the list could not refresh. Retry the connection. Do not create the same lead again.');
        }
      } catch (error) {
        setCreateError(error instanceof Error ? `Lead was not saved: ${error.message}` : 'Lead was not saved. Please retry.');
      } finally {
    ...
    <section className={`content ${referenceDashboard ? 'reference-dashboard-content' : 'unified-content'}`}>
      ...
      {page === 'Lead research' && <LeadResearchQueue viewer={viewer} users={workspaceDirectory} session={session} onNotice={setNotice} onPublished={async (research, salesOwnerId) => {
        if (session) setLeads(await loadRemoteLeads(session));
          ...
          const lead: Lead = { id, name: research.name, phone: research.methods.find((method) => method.type === 'phone')?.value, email: research.methods.find((method) => method.type === 'email')?.value, discoveredContactMethods: research.methods, source: research.source, category: research.category, description: research.description, address: research.address, originalDetails: research.details, researchSummary: research.researchNotes, sourceReceivedAt: research.receivedAt, marketingOwnerId: research.marketingOwnerId, sourceDate: at.slice(0, 10), status: 'assigned', qualification: 'not_available', priority: 0, assignments: [{ id: `assignment-${id}`, ownerId: salesOwnerId, assignedBy: viewer.id, at, visibility: 'full_context', reason: 'Research handoff' }], activities: [leadActivity(viewer.id, 'system', 'Lead published from the synthetic research queue.')], followUps: [], incorrectReports: [] };
...
function LeadTable({ leads, users: directory, onSelect, onCreate, onViewAll }: { leads: Lead[]; users: User[]; onSelect: (id: string) => void; onCreate: () => void; onViewAll?: () => void }) {
  const [query, setQuery] = useState(''); const [status, setStatus] = useState<'all' | OpportunityStatus>('all'); const [source, setSource] = useState('all'); const [owner, setOwner] = useState('all');
  const displayName = (id?: string) => directory.find((user) => user.id === id)?.name ?? 'Unassigned';
  const visibleLeads = useMemo(() => { const term = query.trim().toLowerCase(); return leads.filter((lead) => { const values = [lead.name, lead.phone, lead.email, lead.source, ...((lead.discoveredContactMethods ?? []).map((method) => method.value))].filter(Boolean).join(' ').toLowerCase(); return (!term || values.includes(term)) && (status === 'all' || lead.status === status) && (source === 'all' || lead.source === source) && (owner === 'all' || ownerId(lead) === owner); }); }, [leads, owner, query, source, status]);
  const sources = [...new Set(leads.map((lead) => lead.source))].sort();
  const owners = [...new Set(leads.map((lead) => ownerId(lead)).filter((id): id is string => Boolean(id)))];
  return <article className="panel leads-panel"><div className="panel-heading"><div><span className="eyebrow">LEAD INBOX <em className="preview-badge">Action queue</em></span><h2>Ready for the next action</h2><p>Search the people and contact methods you need, then narrow the work queue.</p></div><div className="lead-table-actions"><button className="primary" onClick={onCreate}>Add lead</button>{onViewAll && <button className="quiet" onClick={onViewAll}>View all leads <IconArrowUpRight size={15} /></button>}</div></div><div className="lead-inbox-filters"><label className="lead-search"><span>Search leads</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name, phone, email or source" /></label><label>Status<select value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="all">All statuses</option>{statusOptions.map((item) => <option value={item} key={item}>{statusLabels[item]}</option>)}</select></label><label>Source<select value={source} onChange={(event) => setSource(event.target.value)}><option value="all">All sources</option>{sources.map((item) => <option value={item} key={item}>{item}</option>)}</select></label><label>Owner<select value={owner} onChange={(event) => setOwner(event.target.value)}><option value="all">All owners</option>{owners.map((item) => <option value={item} key={item}>{displayName(item)}</option>)}</select></label><button type="button" className="quiet" onClick={() => { setQuery(''); setStatus('all'); setSource('all'); setOwner('all'); }}>Reset</button><small>{visibleLeads.length} result{visibleLeads.length === 1 ? '' : 's'}</small></div><div className="lead-table"><div className="lead-row header"><span>Lead</span><span>Owner</span><span>Status</span><span>Next action</span></div>{visibleLeads.length ? visibleLeads.map((lead) => { const next = lead.followUps.find((followUp) => followUp.status === 'open'); const ownerName = displayName(ownerId(lead)); return <button className="lead-row lead-button" key={lead.id} onClick={() => onSelect(lead.id)}><strong><span className="lead-identity"><span className="avatar avatar-lead">{initialsFor(lead.name)}</span><span>{lead.name}<small>{lead.source}</small></span></span></strong><span className="owner-identity"><span className="avatar">{initialsFor(ownerName)}</span>{ownerName}</span><span className={`status ${lead.status}`}>{statusLabels[lead.status]}</span><span>{relativeDue(next)}</span></button>; }) : <p className="empty">No leads match these filters.</p>}</div></article>;
}
...
function CreateLead({ error, saving, viewer, salesAgents, crmReady, onReconnect, onClose, onSubmit }: { error: string; saving: boolean; viewer: User; salesAgents: User[]; crmReady: boolean; onReconnect?: () => void; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) { const allowed = viewer.role === 'admin' || viewer.role === 'marketer'; return <div className="modal-backdrop" role="presentation"><form className="modal lead-form-modal" onSubmit={onSubmit}><div className="panel-heading"><div><span className="spark dark">NEW LEAD</span><h2>Create and route a lead</h2><p>Enter the lead details, then send it directly to Sales when it is ready.</p></div><button type="button" className="quiet" onClick={onClose}>Close</button></div>{allowed ? <>{error && <p className="warning" role="alert">{error}</p>}<div className="lead-form-guidance"><b>Required to create:</b> name and at least one phone number or email address. You can add more contact methods from the lead page.</div>{!crmReady && <div className="warning crm-reconnect-warning"><span>The CRM connection is unavailable. Reconnect before creating a lead so no assignment is lost.</span>{onReconnect && <button type="button" className="quiet" onClick={() => { onClose(); onReconnect(); }}>Reconnect</button>}</div>}<div className="lead-form-sections"><fieldset><legend>1. Lead and project</legend><div className="detail-grid"><label>Name<input name="name" required autoComplete="name" /></label><label>Source<select name="source" defaultValue="" required><option value="" disabled>Choose source</option>{sourceOptions.map((source) => <option key={source} value={source}>{source}</option>)}</select></label><label>Lead category<select name="category" defaultValue="not_available" required>{leadCategoryOptions.map((category) => <option key={category} value={category}>{leadCategoryLabels[category]}</option>)}</select></label><label className="form-span">Description<textarea name="description" rows={4} maxLength={4000} placeholder="Project needs, background, budget or useful context." /></label></div></fieldset><fieldset><legend>2. Contact details</legend><div className="detail-grid"><label>Phone number<input name="phone" type="tel" inputMode="tel" autoComplete="tel" placeholder="Optional if email is provided" /></label><label>Email address<input name="email" type="email" inputMode="email" autoComplete="email" placeholder="Optional if phone is provided" /></label></div></fieldset><fieldset><legend>3. Sales handoff</legend><label>Assign Sales Agent<select name="owner" defaultValue=""><option value="">Save as unassigned</option>{salesAgents.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label><small>The selected Sales Agent can see the lead immediately after it is saved.</small></fieldset></div><button className="primary" disabled={!crmReady || saving}>{saving ? 'Saving…' : 'Create lead and save'}</button></> : <p className="warning">Only Admin and Marketing can create leads.</p>}</form></div>; }
