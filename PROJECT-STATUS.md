...
### Gmail HTML recovery: September 24

Acceptance documentation and the standalone bell test are published in [PR49](https://github.com/CharlesSmith999/elevanta-ai/pull/49), awaiting merge. A subsequent browser recheck timed out in automatic approval twice and the GitHub connector became unavailable. Those access issues do not change the verified PR48 production results below; no later live count is claimed.

Released and verified: PR [48](https://github.com/CharlesSmith999/elevanta-ai/pull/48), production commit `ca4396cca839fb2f14d8c541ae7749df3511ad40`, existing Vercel deployment `CPZPFd7jir2uvPwxn5o6sC8xC2av`. The original 35 held messages grew to 82 before release; all 82 were recovered into Research. Live counts: 82 parsed messages, 82 research leads, zero pending parser review, zero duplicate message groups. Migration `202609240001` was applied after deployment and recorded once in the canonical ledger. Marketing role RLS returned all 82; Sales role RLS returned zero. The live Marketing preview displayed the leads without mailbox settings. The one-minute scheduler remained active with HTTP 200 and `delivered` at 21:13 UTC. All 89 application tests, production build, and all 24 isolated migrations passed. See [GMAIL-HTML-RECOVERY.md](./docs/GMAIL-HTML-RECOVERY.md). This verified incident record supersedes older activation-in-progress snapshots below.

### New-work bell approved, v1.13

Deployed with PR48. Existing role-scoped APIs supply a 15-second open-app check, 15% to 100% volume with no in-app mute, initial-load suppression, best-effort browser-tab deduplication, and a visual count. The live Test bell control reached active audio state and the slider stopped at 15%. The component harness passed Research arrival, pending autoplay, repeat suppression, corrupted storage, Sales own-assignment scope, no Sales Research queries, and Marketing Manager eligibility. See [CRM-DECISIONS-v1.13.md](./CRM-DECISIONS-v1.13.md) and `scripts/test-lead-bell.mjs`. Actual speaker loudness and simultaneous separate-user audible delivery were not measured; browser/OS sound settings remain outside application control.
...
The supplied Apps Script has now been reviewed. Gmail parsing, a read-only reader, OAuth/PKCE connection, encrypted refresh-token storage, worker leases/cursor/retry handling, daily scheduler and Admin connection/activation/sync/health controls are implemented locally. Updated validation: 78 application tests pass; all 21 migrations replay; worker lease exclusion, credential-table denial and idempotent unowned intake tests pass. Both typechecks and production build pass. These changes are not deployed and live end-to-end OAuth/ingestion is not verified.

Google Cloud setup is blocked by required 2-step verification on the signed-in account. The account owner must complete this. OAuth client configuration, server secrets, production migrations, consent and a real-message acceptance test remain required. No Gmail connection or automatic intake has been activated. See [GMAIL-INTAKE-IMPLEMENTATION.md](./docs/GMAIL-INTAKE-IMPLEMENTATION.md).

Decision v1.8 adds a Marketing-only Lead Research Queue for masked Gmail leads. Safe-fixture development is approved now. Sales cannot see research records. Marketing must record a usable unmasked contact method and explicitly publish the item to an active Sales Agent before it enters the existing Lead Inbox. Live Gmail activation remains gated by current Apps Script review plus the outstanding Xaviar Admin and Manager approvals.
...
| Inbound Lead Research v1.8 | Research queue deployed; Gmail ingestion pending | PR #43 merged as `aea59648979ca682ed6664f914d30a7239cc7a77`. Vercel production deployment `G97XHfkg82jfTUsKu9u1FPVtcukZ` succeeded. 67 application tests, typechecks and build passed; all 19 migrations replayed in isolated PostgreSQL. Both research migrations are confirmed in the production ledger. Rollback-only production handoff and idempotency checks passed. Signed-in Admin queue and directory loaded without browser errors. | Gmail connector/parser implementation requires the existing Apps Script, redacted email samples, and mailbox/label scope. Live activation also requires the documented Admin/Manager approvals. No live Gmail ingestion has been implemented or tested. See the test checklist for remaining acceptance coverage. |

## 2026-09-18 inbound research release verification

- Production: https://elevanta-ai-pipeline.vercel.app/ . Git release: PR #43, `aea59648979ca682ed6664f914d30a7239cc7a77`.
- GitHub Actions run 309 passed for released source `38ec857`; Vercel preview and production succeeded.
- `/api/health` returned `status: ok`; `/api/ready` returned `configuration: valid`. The authenticated Admin research queue loaded its empty database result without falling back to sample leads or showing connection errors.
- Migrations `202609170001` and `202609180001` are applied and recorded. Production rollback-only test created and published a synthetic candidate, preserved both contact methods, and returned the same opportunity on retry. The transaction was rolled back.
- No Gmail account was connected, no outbound messages were sent, and no workbook migration was performed. An empty research queue is expected until approved ingestion is implemented.
- Release evidence documentation is retained locally for the next grouped documentation publication, avoiding a separate documentation-only production deployment.
