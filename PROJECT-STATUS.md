...
| Inbound Lead Research v1.8 | Research workflow tested; release in progress; Gmail ingestion pending | Marketing queue, research editing, scoped API/database access, explicit Sales handoff, synthetic handoff, and duplicate guards are implemented. 67 application tests pass. All 18 migrations replay in an isolated PostgreSQL runtime and database handoff/permission checks pass, including Marketing Manager publication and preserving multiple methods. | Finish release verification. Gmail connector/parser implementation and activation require the existing Apps Script and mailbox scope. No live Gmail ingestion has been implemented or tested. |
