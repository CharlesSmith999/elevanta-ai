...
| 1 — Foundation | Authentication implementation added; production setup pending | React/Vite web app, Node API, Supabase migrations (including stage history), role/manager model, audit-oriented workflow functions, protected lead actions, test accounts, GitHub CI workflow, Vercel catch-all API entry point, typed API contracts, role-safe dashboard routes, and an environment-gated Supabase email/password sign-in screen are implemented. Safe demo mode remains available only when Supabase web variables are absent. | Configure and verify the approved Supabase Auth project, administrator/test profiles, Vercel variables, signed-in route behavior, and formal Milestone 1 approval. |
