...
Google Cloud security access is restored. On 2026-09-22, the owner explicitly approved OAuth credential creation and storage. The client is created; production-only secret values are saved in the existing Vercel project, and the scheduler credential is saved as `elevanta_gmail_cron_secret` in existing Supabase Vault. See [GMAIL-INTAKE-IMPLEMENTATION.md](./GMAIL-INTAKE-IMPLEMENTATION.md) for the value-free inventory. Do not duplicate the OAuth secret or encryption key in ordinary database tables, expose them to Preview/browser code, or replace keys on subsequent runs. Configuration still requires deployment; mailbox consent, minute scheduling and live-message acceptance remain outstanding. Keep intake disabled until the release gates are met.
